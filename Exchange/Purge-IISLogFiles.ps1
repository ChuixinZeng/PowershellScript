<#
  ...
  .EXAMPLE
  Delete Exchange and IIS log files older than 7 days with automatic discovery 【集团邮件60天前日志清楚】

  .\Purge-LogFiles -DaysToKeep 60 -Auto

  .EXAMPLE
  Delete Exchange and IIS log files older than 7 days with automatic discovery and send email report

  .\Purge-LogFiles -DaysToKeep 180 -Auto -SendMail -MailFrom exadmin@alarm.360.cn -MailTo exadmin@alarm.360.cn -MailServer mail.alarm.360.cn
  .\Purge-LogFiles -DaysToKeep 180 -Auto -SendMail -MailFrom exadmin@360jinrong.net -MailTo "exadmin@360jinrong.net,ZHAOYACHAO@360.CN" -MailServer JKCAS01.jk.360jinkong.cn
  .\Purge-LogFiles -DaysToKeep 180 -Auto -SendMail -MailFrom exadmin@360jinrong.net -MailTo exadmin@360jinrong.net -MailServer JKCAS01.jk.360jinkong.cn
