				...
				IF ($PSCmdlet.ShouldProcess($Computer, "Enable Remote Desktop via Win32_TerminalServiceSetting"))
				{
					TRY
					{
						...
						IF (Test-Connection -Computer $Computer -count 1 -quiet)
						{
							...
							#后面之所以跟上out-null，一方面屏幕不会返回设置信息，另外一方面保证输出到richtext的信息只有开启成功的提示信息
							(Get-WmiObject -Class Win32_TSGeneralSetting -Namespace "root\cimv2\terminalservices").SetUserAuthenticationRequired(0) | Out-Null
							
							$returnsetremotesuccessmessage = "本机" + "$Computer" + "远程桌面端口已开启成功！" + "`n"
							$returnsetremotesuccessmessage | Out-String
						}
					} #TRY
					CATCH
					{
