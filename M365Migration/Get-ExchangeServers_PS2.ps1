...
$ReportFile = $CurrentDate+"_ExchangeServerReport.csv"
$ReportPath = $ReportFolderValidated+$ReportFile

### Store the input CSV in a variable
$InputList = Get-ExchangeServer | Select -ExpandProperty Name

### Gather Active mailbox statistics
ForEach($Entry in $InputList)
{
    Write-Host "Now gathering mailbox databases on server $Entry"
    Get-ExchangeServer |
    Select Name,Domain,Edition,Fqdn,IsHubTransportServer,IsClientAccessServer,IsExchange2007OrLater,IsEdgeServer,IsMailboxServer,IsE14OrLater,IsE15OrLater,IsUnifiedMessagingServer,IsFrontendTransportServer,AdminDisplayVersion,Site,ServerRole,ProductID,IsExchangeTrialEdition,IsExpiredExchangeTrialEdition |
