...
$ReportFile = $CurrentDate+"_MailboxDatabaseReport.csv"
...
ForEach($Entry in $InputList)
{
    ...
    Get-MailboxDatabase -Server $Entry |
    Select ServerName,Server,Name,Mounted,DatabaseGroup,IsMailboxDatabase,IsPublicFolderDatabase,PublicFolderDatabase,IndexEnabled,DatabaseCreated,EdbFilePath,AdminDisplayVersion,AdminDisplayName,ExchangeVersion,WhenCreated,WhenChanged |
