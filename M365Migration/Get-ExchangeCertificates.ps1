...
$ReportFile = $CurrentDate+"_ExchangeCertificateReport.csv"
...
ForEach($Entry in $InputList)
{
    ...
    Get-ExchangeCertificate -Server $Entry |
    Select FriendlyName,Thumbprint,Archived,IsSelfSigned,HasPrivateKey,DnsNameList,Services,Status,NotAfter,Subject,Issuer |
