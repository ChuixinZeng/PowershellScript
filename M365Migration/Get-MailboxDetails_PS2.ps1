...
$ReportFile = $CurrentDate+"_MailboxReport.csv"
$ReportPath = $ReportFolderValidated+$ReportFile

### Gather mailbox details and store in a variable
$InputList = Get-Mailbox -ResultSize Unlimited |
Select -ExpandProperty PrimarySmtpAddress |
%{[string] $PrimarySmtpAddressString = $_ ; $PrimarySmtpAddressString}

$y = Get-Mailbox -Identity $UID | Select-object -ExpandProperty primarysmtpaddress | %{[string] $x = $_ ; $x}

### Gather Active mailbox statistics
ForEach($Entry in $InputList)
{
    $PrimarySmtpAddress = $Entry
    Write-Host "Now gathering mailbox statistics for $PrimarySmtpAddress"
    $TotalItemSize = Get-MailboxStatistics -Identity $PrimarySmtpAddress |
    Select -ExpandProperty TotalItemSize |
    Select -ExpandProperty Value
    $TotalDeletedItemSize = Get-MailboxStatistics -Identity $PrimarySmtpAddress |
    Select -ExpandProperty TotalDeletedItemSize |
    Select -ExpandProperty Value
    $ItemCount = Get-MailboxStatistics -Identity $PrimarySmtpAddress |
    Select -ExpandProperty ItemCount
    $DeletedItemCount = Get-MailboxStatistics -Identity $PrimarySmtpAddress |
    Select -ExpandProperty DeletedItemCount
    Write-Host "Now retrieving mailbox details for $PrimarySmtpAddress"
    Get-Mailbox -Identity $PrimarySmtpAddress |
    Select UserPrincipalName,PrimarySmtpAddress,@{Name=”EmailAddresses";Expression={$_.EmailAddresses -Join “;”}},LegacyExchangeDN,Alias,ExternalDirectoryObjectId,ArchiveState,LitigationHoldEnabled,SingleItemRecoveryEnabled,RetentionHoldEnabled,IsLinked,IsShared,IsResource,RetentionPolicy,SharingPolicy |
    Add-Member -MemberType NoteProperty -Name TotalItemSize -Value $TotalItemSize -PassThru -Force |
    Add-Member -MemberType NoteProperty -Name TotalDeletedItemSize -Value $TotalDeletedItemSize -PassThru -Force |
    Add-Member -MemberType NoteProperty -Name ItemCount -Value $ItemCount -PassThru -Force |
    Add-Member -MemberType NoteProperty -Name DeletedItemCount -Value $DeletedItemCount -PassThru -Force |
